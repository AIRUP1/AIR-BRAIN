...
const availabilityWindowSchema = z.object({ weekday: z.number().int().min(0).max(6), startMinutes: z.number().int().min(0).max(1_439), endMinutes: z.number().int().min(1).max(1_440), timeZone: z.string().min(3).max(80) }).refine((window) => window.startMinutes < window.endMinutes, { message: "Availability end time must be after the start time." });
...
export const appRouter = router({
  ...
  studio: router({
    ...
    pos: router({
      ...
      listTickets: publicProcedure.input(z.object({ brandKitId: z.number().int().positive(), search: z.string().max(64).optional(), status: z.enum(["quote", "exported", "void"]).optional(), from: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), to: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), limit: z.number().int().min(1).max(100).optional() })).query(({ input }) => db.listPosTickets(input)),
      ...
      addTechnician: publicProcedure.input(z.object({ brandKitId: z.number().int().positive(), name: z.string().min(2).max(160), role: z.string().max(120).optional(), color: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional() })).mutation(async ({ input }) => ({ id: await db.createTechnician(input) ?? 0 })),
      listTechnicians: publicProcedure.input(z.object({ brandKitId: z.number().int().positive() })).query(({ input }) => db.listTechnicians(input.brandKitId)),
      listTechnicianAvailability: publicProcedure.input(z.object({ brandKitId: z.number().int().positive() })).query(({ input }) => db.listTechnicianAvailability(input.brandKitId)),
      setTechnicianAvailability: publicProcedure.input(z.object({ brandKitId: z.number().int().positive(), technicianId: z.number().int().positive(), windows: z.array(availabilityWindowSchema).min(1).max(14) })).mutation(({ input }) => db.replaceTechnicianAvailability(input)),
      checkAppointmentAvailability: publicProcedure.input(z.object({ brandKitId: z.number().int().positive(), technicianId: z.number().int().positive(), startsAt: z.string().datetime(), durationMinutes: z.number().int().min(15).max(720) })).query(({ input }) => db.checkAppointmentAvailability({ ...input, startsAt: new Date(input.startsAt) })),
      createAppointment: publicProcedure.input(z.object({ brandKitId: z.number().int().positive(), ticketId: z.number().int().positive(), technicianId: z.number().int().positive(), startsAt: z.string().datetime(), durationMinutes: z.number().int().min(15).max(720), notes: z.string().max(500).optional() })).mutation(async ({ input }) => ({ id: await db.createAppointment({ ...input, startsAt: new Date(input.startsAt) }) ?? 0 })),
      listAppointments: publicProcedure.input(z.object({ brandKitId: z.number().int().positive() })).query(({ input }) => db.listAppointments(input.brandKitId)),
      updateAppointmentStatus: publicProcedure.input(z.object({ appointmentId: z.number().int().positive(), brandKitId: z.number().int().positive(), status: z.enum(["scheduled", "confirmed", "completed", "cancelled"]) })).mutation(({ input }) => db.updateAppointmentStatus(input)),
