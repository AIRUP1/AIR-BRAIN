...
The reviewed durable-data foundation is in [supabase/migrations/20260925_000001_air_agents_initial.sql](supabase/migrations/20260925_000001_air_agents_initial.sql). It creates the Supabase Postgres schema, RLS policies, audit trail, and GitHub delivery receipt table, but it is **not applied** and the current runtime remains on the isolated SQLite adapter until the selected Supabase project is authorized and the production repository adapter is enabled.
